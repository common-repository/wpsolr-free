...
class WPSOLR_Admin_UI_Ajax_Index_Configuration_Default_builder extends WPSOLR_Admin_UI_Ajax_Search {
	...
	public static function extract_parameters() {
		...
		$parameters = array(
			self::PARAMETER_OPTION_NAME      => sanitize_text_field( empty( $_GET[ self::PARAMETER_OPTION_NAME ] ) ? '' : $_GET[ self::PARAMETER_OPTION_NAME ] ),
			self::PARAMETER_INDEX_UUID       => sanitize_text_field( empty( $_GET[ self::PARAMETER_INDEX_UUID ] ) ? '' : $_GET[ self::PARAMETER_INDEX_UUID ] ),
			self::PARAMETER_CONFIGURATION_ID => sanitize_text_field( empty( $_GET[ self::PARAMETER_CONFIGURATION_ID ] ) ? '' : $_GET[ self::PARAMETER_CONFIGURATION_ID ] ),
			self::PARAMETER_BUILDER_ID       => sanitize_text_field( empty( $_GET[ self::PARAMETER_BUILDER_ID ] ) ? '' : $_GET[ self::PARAMETER_BUILDER_ID ] ),
		);
