...
class WPSOLR_Model_Meta_Type_Post extends WPSOLR_Model_Meta_Type_Abstract {
	...
	public function get_indexing_sql( $debug_text, $batch_size = 100, $post = null, $is_debug_indexing = false, $is_only_exclude_ids = false, $last_post_date_indexed = null ) {
		...
		$post_type_is_admin = WPSOLR_Service_Container::getOption()->get_option_index_post_type_is_admin( $post_type );
		...
		if ( isset( $where_p ) ) {
			...
			if ( $post_type_is_admin ) {
				...
				$index_post_statuses = implode( ',', apply_filters( WPSOLR_Events::WPSOLR_FILTER_POST_STATUSES_TO_INDEX, [], $post_type ) );
				if ( ! empty( $index_post_statuses ) ) {
					...
					//$query_where_stmt    = "post_status NOT IN ('$index_post_statuses') AND ( $where_p )";

					$query_where_stmt = "$where_p";
				}

			} else {
				// Only published status by default (for searc on front only)
