...
class WPSOLR_Option_Indexes extends WPSOLR_Extension {
	...
	public function get_search_engine_name( $search_engine ) {
		...
		if ( WPSOLR_AbstractEngineClient::get_engines_definitions()[ $search_engine ]['has_personalized_search'] ?? true ) {
			$features[] = 'Personalized Search';
		}
		if ( WPSOLR_AbstractEngineClient::get_engines_definitions()[ $search_engine ]['has_recommendations'] ?? true ) {
			$features[] = 'Recommendations';
		}

		return sprintf( '%s from %s', implode( ', ', $features ), $result );
	}
	...
	public function build_config( &$solr_index_indice, $language_code, $timeout ) {
		...
		$config = [
			...
			'extra_parameters'      => [
				'index_email'                                    => $solr_index['index_aws_region'] ?? '',
				...
				'dataset_events_arn'                             => $solr_index['dataset_events_arn'] ?? '',
