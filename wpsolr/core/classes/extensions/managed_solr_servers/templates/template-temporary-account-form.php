                ...
                <div class='col_right' style='width: 80%;'>
                    <ul>
						...
						foreach ( OptionManagedSolrServer::get_managed_solr_services() as $list_managed_solr_service_id => $managed_solr_service ) {
							printf( "<li><input type='radio' name='managed_solr_service_id' value='%s' %s/><a href='%s' target='_blank'>%s</a></li>",
								$list_managed_solr_service_id,
								checked( $list_managed_solr_service_id, OptionManagedSolrServer::HOSTING_ID_ELASTIC_CLOUD, false ), //checked( $list_managed_solr_service_id, $managed_solr_service_id, false ),
								$managed_solr_service[ OptionManagedSolrServer::MANAGED_SOLR_SERVICE_HOME_PAGE ],
