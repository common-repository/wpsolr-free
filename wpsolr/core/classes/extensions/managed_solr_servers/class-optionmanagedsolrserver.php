 ...
 */
class OptionManagedSolrServer extends WPSOLR_Extension {
	 ...
	 * Hosting service IDs
	 */
	const HOSTING_ID_ELASTIC_CLOUD = 'elastic_cloud';
	...
	public static function set_response_ko( $response_object ) {

		$response_object->status->state = 'KO';
	}
	...
	public static function set_response_error_message( $response_object, $message ) {

		$response_object->status->message = $message;
	}

	public static function get_response_result( $response_object, $field ) {

		$result = isset( $response_object->results ) && isset( $response_object->results[0] )
	...
	public static function set_response_result( $response_object, $field, $value ) {

		if ( isset( $response_object->results ) && isset( $response_object->results[0] ) ) {
			if ( is_array( $response_object->results[0] ) ) {

				$response_object->results[0][0]->$field = $value;

			} else {

				$response_object->results[0]->$field = $value;
			}
		}
	...
	public function call_rest_create_solr_index( $g_recaptcha_response ) {
		...
		$response_object = $this->call_rest_post(
			...
			array(
				...
				'remoteip' => WPSOLR_Sanitize::sanitize_text_field( $_SERVER['REMOTE_ADDR'] )
			)
		);
	...
	public static function get_managed_solr_services() {
		...
		$result[ self::HOSTING_ID_ELASTIC_CLOUD ] = [
			self::MANAGED_SOLR_SERVICE_SEARCH_ENGINE                      => WPSOLR_AbstractSearchClient::ENGINE_ELASTICSEARCH,
			self::MANAGED_SOLR_SERVICE_LABEL                              => 'Elastic Cloud',
			self::MANAGED_SOLR_SERVICE_HOME_PAGE                          => 'https://www.elastic.co/cloud',
			self::MANAGED_SOLR_SERVICE_API_PATH                           => 'https://api.wpsolr.com/v1/partners/24b7729e-02dc-47d1-9c15-f1310098f93f',
			self::MANAGED_SOLR_SERVICE_CHANNEL_ORDER_URL                  => 'https://api.wpsolr.com/v1/providers/8c25d2d6-54ae-4ff6-a478-e2c03f1e08a4/accounts/24b7729e-02dc-47d1-9c15-f1310098f93f/addons/71bc4b32-be17-4012-b4e9-299d47d7f86d/order-solr-index/eafe62fb-1ee1-4a64-ac33-b2b7b045feb0',
			self::MANAGED_SOLR_SERVICE_CHANNEL_GOOGLE_RECAPTCHA_TOKEN_URL => 'https://api.wpsolr.com/v1/providers/8c25d2d6-54ae-4ff6-a478-e2c03f1e08a4/accounts/24b7729e-02dc-47d1-9c15-f1310098f93f/addons/71bc4b32-be17-4012-b4e9-299d47d7f86d/google-recaptcha-token',
		...
		$result['gotosolr'] = [
			...
			self::MANAGED_SOLR_SERVICE_LABEL                              => 'Gotosolr (unavailable)',
			self::MANAGED_SOLR_SERVICE_HOME_PAGE                          => 'http://www.gotosolr.com/en',
			self::MANAGED_SOLR_SERVICE_API_PATH                           => 'https://api.wpsolr.com/v1/partners/24b7729e-02dc-47d1-9c15-f1310098f93f',
			self::MANAGED_SOLR_SERVICE_CHANNEL_ORDER_URL                  => 'https://api.wpsolr.com/v1/providers/8c25d2d6-54ae-4ff6-a478-e2c03f1e08a4/accounts/24b7729e-02dc-47d1-9c15-f1310098f93f/addons/f8622320-5a3b-48cf-a331-f52459c46573/order-solr-index/8037888b-501a-4200-9fb0-b4266434b161',
			self::MANAGED_SOLR_SERVICE_CHANNEL_GOOGLE_RECAPTCHA_TOKEN_URL => 'https://api.wpsolr.com/v1/providers/8c25d2d6-54ae-4ff6-a478-e2c03f1e08a4/accounts/24b7729e-02dc-47d1-9c15-f1310098f93f/addons/f8622320-5a3b-48cf-a331-f52459c46573/google-recaptcha-token',
			self::MANAGED_SOLR_SERVICE_ORDERS_URLS                        => [
				[
