...
<div class="wdm_row" style="top-margin:5px;">
    <div class='col_left'>
		<?php WPSOLR_Escape::echo_escaped( $license_manager->show_premium_link( true, OptionLicenses::LICENSE_PACKAGE_PREMIUM, 'Layout', true ) ); ?>
    </div>
    <div class='col_right'>
        <select name='<?php WPSOLR_Escape::echo_esc_attr( $view_options_name ); ?>[<?php WPSOLR_Escape::echo_esc_attr( WPSOLR_Option::OPTION_FACET_FACETS_LAYOUT ); ?>][<?php WPSOLR_Escape::echo_esc_attr( $selected_val ); ?>]'
                class="wpsolr-remove-if-empty wpsolr_layout_select"
                data-wpsolr-empty-value="<?php WPSOLR_Escape::echo_esc_attr( WPSOLR_UI_Layout_Check_Box::CHILD_LAYOUT_ID ); ?>">
			...
			foreach ( $facets_layout_available as $layout_id => $layout_class ) { ?>
                <option value="<?php WPSOLR_Escape::echo_esc_attr( $layout_id ); ?>" <?php selected( $current_layout_id, $layout_id ); ?> <?php disabled( ! $layout_class::get_is_enabled() ); ?>>
					<?php WPSOLR_Escape::echo_escaped( sprintf( '%s %s', WPSOLR_Escape::esc_html( $layout_class::get_label() ), $layout_class::get_is_enabled() ? '' : ' (coming soon)' ) ); ?>
                </option>
			<?php } ?>
        </select>
		...
		if ( file_exists( $file_to_include = apply_filters( WPSOLR_Events::WPSOLR_FILTER_INCLUDE_FILE, WPSOLR_Help::HELP_FACET_SHOW_WOOCOMMERCE_VARIATION_IMAGE ) ) ) {
			require $file_to_include;
		}
		...
		include 'facet-theme-layout-feature-hide-if-no-choice.inc.php';
		include 'facet-theme-layout-feature-can-not-exist.inc.php';
		...
		include 'facet-theme-layout-feature-show-all-link.inc.php';
