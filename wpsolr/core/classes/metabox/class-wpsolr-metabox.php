...
class WPSOLR_Metabox {
	...
	public function action_add_meta_boxes_callback( $post ) {
        ...
        <div class="wpsolr-metabox-row-content">
            <label for="<?php WPSOLR_Escape::echo_esc_attr( self::METABOX_FIELD_IS_DO_NOT_INDEX ); ?>">
                <input type="checkbox"
                       name="<?php WPSOLR_Escape::echo_esc_attr( self::METABOX_FIELD_IS_DO_NOT_INDEX ); ?>"
                       id="<?php WPSOLR_Escape::echo_esc_attr( self::METABOX_FIELD_IS_DO_NOT_INDEX ); ?>"
                       value="<?php WPSOLR_Escape::echo_esc_attr( self::METABOX_CHECKBOX_YES ); ?>" <?php if ( isset ( $post_meta[ self::METABOX_FIELD_IS_DO_NOT_INDEX ] ) ) {
		...
		?>
		<div class="wpsolr-metabox-row-content">
			<label for="<?php WPSOLR_Escape::echo_esc_attr( self::METABOX_FIELD_IS_DO_INDEX_EMBED_ANY_DOCUMENT ); ?>">
				<input type="checkbox"
				       name="<?php WPSOLR_Escape::echo_esc_attr( self::METABOX_FIELD_IS_DO_INDEX_EMBED_ANY_DOCUMENT ); ?>"
				       id="<?php WPSOLR_Escape::echo_esc_attr( self::METABOX_FIELD_IS_DO_INDEX_EMBED_ANY_DOCUMENT ); ?>"
				       value="<?php WPSOLR_Escape::echo_esc_attr( self::METABOX_CHECKBOX_YES ); ?>" <?php if ( isset ( $post_meta[ self::METABOX_FIELD_IS_DO_INDEX_EMBED_ANY_DOCUMENT ] ) ) {
			checked( $post_meta[ self::METABOX_FIELD_IS_DO_INDEX_EMBED_ANY_DOCUMENT ][0], self::METABOX_CHECKBOX_YES );
		} ?>
					<?php WPSOLR_Escape::echo_esc_attr( $license_manager->get_license_enable_html_code( OptionLicenses::LICENSE_PACKAGE_EMBED_ANY_DOCUMENT )); ?>
				/>
				<?php WPSOLR_Escape::echo_esc_none( $license_manager->show_premium_link( true, OptionLicenses::LICENSE_PACKAGE_EMBED_ANY_DOCUMENT, _x( 'Search in Embed Any Document', 'wpsolr' ), true, true ) ); ?>
			</label>
		</div>
		*/
		?>
	...
	public function get_is_show_meta_box( $post ) {
		...
		if ( ! in_array( $type, $types, true ) ) {
			...
			WPSOLR_Escape::echo_esc_html( $message ) . ' You can change that in wpsolr settings.';
	...
	public function action_save_post_callback( $post_id ) {
		...
		update_post_meta( $post_id, self::METABOX_FIELD_IS_DO_NOT_INDEX, isset( $_POST[ self::METABOX_FIELD_IS_DO_NOT_INDEX ] ) ? WPSOLR_Sanitize::sanitize_text_field( $_POST[ self::METABOX_FIELD_IS_DO_NOT_INDEX ] ) : '' );
		update_post_meta( $post_id, self::METABOX_FIELD_IS_DO_INDEX_ACF_FIELD_FILES, isset( $_POST[ self::METABOX_FIELD_IS_DO_INDEX_ACF_FIELD_FILES ] ) ? WPSOLR_Sanitize::sanitize_text_field( $_POST[ self::METABOX_FIELD_IS_DO_INDEX_ACF_FIELD_FILES ] ) : '' );
		update_post_meta( $post_id, self::METABOX_FIELD_IS_DO_INDEX_TOOLSET_FIELD_FILES, isset( $_POST[ self::METABOX_FIELD_IS_DO_INDEX_TOOLSET_FIELD_FILES ] ) ? WPSOLR_Sanitize::sanitize_text_field( $_POST[ self::METABOX_FIELD_IS_DO_INDEX_TOOLSET_FIELD_FILES ] ) : '' );
