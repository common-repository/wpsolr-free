...
abstract class WPSOLR_UI_Layout_Abstract implements WPSOLR_UI_Layout_Interface {
	...
	const FEATURE_SIZE_SHOW_ALL_LINK = 'feature_size_show';
	const FEATURE_IS_HIDE_IF_NO_CHOICE = 'feature_is_hide_if_no_choice';
	...
	public function load_layout_skin( $skin_id, $facet_class_uuid ) {
		...
		if ( defined( 'WPSOLR_PLUGIN_DIR' ) ) {
			...
			$dir = WPSOLR_PLUGIN_DIR . '/wpsolr/pro/extensions/theme/xxx';
	...
	public function displayFacetHierarchy( $facet_class_uuid, $facet_template, $facet_grid_class, &$html, &$facet, &$items, $level = 0 ) {
		...
		foreach ( $items as &$item ) {
			...
			$facet_label        = '';
			$facet_data         = [];
			$facet_data['id']   = $facet_id;
			$facet_data['type'] = $data_facet_type;
			//$facet_data['is_permalink'] = isset( $facet['is_permalink'] ) ? $facet['is_permalink'] : false;
			$facet_data['level'] = $level;
			...
			$facet_data['item_value'] = str_replace( "'", '&#39;', $facet_data['item_value'] ); // important, for "'" contained in json
			$facet_data['item_value'] = str_replace( '&quot;', '"', $facet_data['item_value'] ); // important, for "'" contained in json
			$facet_data['html_data']  = $facet_data;
			 ...
			 * Copy back to $item
			 */
			$item['html_data']   = $facet_data['html_data'];
			$item['level']       = $level;
			$item['class_level'] = $facet_level;
			$item['is_selected'] = $item_selected;
			$item['content']     = ( empty( $item['items'] ) ? $facet_label : $item_localized_name ); // only show count on leaf items (else count is false);


			$this->html_add_facet_item( $html, $level, $facet_layout_id, $item_localized_name, $item, $facet_label, $facet_data, $item_selected, $item_facet_class, $facet_level, $facet_id, $item_value, $facet_data['html_data'] );
