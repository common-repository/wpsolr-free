...
class WPSOLR_UI_Layout_Select extends WPSOLR_UI_Layout_Abstract {
	...
	protected function child_facet_header( &$html, &$items, &$facet, $level ) {
		...
		if ( 0 === $level ) {
			...
			$facet['inner_class'] = $this->get_inner_class( $is_multiple );

			//$html .= "<select {$facet_size_property} class='{$facet['inner_class']}' {$html_property_multiple} data-placeholder='{$facet_placeholder}'>";
		}
