...
class WPSOLR_Search_Weaviate_Client extends WPSOLR_AbstractSearchClient {
	...
	const FILTER_LESSTHAN_ = 'LessThan';
		...
		if ( ! empty( $recommendation ) ) {
			...
			if ( $this->get_wpsolr_query()->wpsolr_get_is_suggestion_type_question_answer() ) {
				...
				switch ( $this->filter ) {
					...
					default:
						throw new \Exception( sprintf( 'Please select a Weaviate filter on screen 2.1 for this view. Current filter "%s" is not implemented', $this->filter ) );
				}
			}
		}
			...
			foreach ( $queries as &$query ) {
				foreach (
					[
						...
						static::FILTER_GREATERTHANEQUAL => static::FILTER_LESSTHAN_,
					] as $replace => $replaced
				) {
	...
	protected function _create_filter_term( $field_name, $field_value, $field_type = '' ): string {
		...
		switch ( $field_type = empty( $field_type ) ? WpSolrSchema::get_custom_field_dynamic_type( $field_name ) : $field_type ) {
			...
			case WpSolrSchema::_SOLR_DYNAMIC_TYPE_INTEGER_LONG:
				$result = $this->_create_single_filter_value_int( $converted_field_name, static::FILTER_EQUAL, static::FILTER_VALUEINT, $field_value );
				break;

			case WpSolrSchema::_SOLR_DYNAMIC_TYPE_DATE:
				$result = $this->_create_single_filter_value_string( $converted_field_name, static::FILTER_EQUAL, static::FILTER_VALUEDATE, "$field_value" );
				break;

			case WpSolrSchema::_SOLR_DYNAMIC_TYPE_FLOAT:
			case WpSolrSchema::_SOLR_DYNAMIC_TYPE_FLOAT_DOUBLE:
				$result = $this->_create_single_filter_value_float( $converted_field_name, static::FILTER_EQUAL, static::FILTER_VALUENUMBER, $field_value );
				break;
			...
			case WpSolrSchema::_SOLR_DYNAMIC_TYPE_STRING1:
				$result = $this->_create_single_filter_value_string( $converted_field_name, static::FILTER_EQUAL, static::FILTER_VALUESTRING, "$field_value" );
				break;

			case WpSolrSchema::_SOLR_DYNAMIC_TYPE_TEXT:
				$result = $this->_create_single_filter_value_string( $converted_field_name, static::FILTER_EQUAL, static::FILTER_VALUETEXT, "$field_value" );
				break;
	...
	public function _create_filter_range( $upper_operation, $filter_name, $field_name, $facet_is_or, $range_start, $range_end, $is_date, $filter_tag = '' ) {
		...
		if ( ( '*' !== $range_start ) && ( '*' !== $range_end ) ) {
			/**
			 * As Weaviate cannot stand (A >= 1 AND A <= 2) OR (A >= 2 AND A <= 3), we use instead (A:1 TO 1.99999) OR (A:2 TO 2.999999)
			 * https://www.weaviate.com/doc/api-reference/api-parameters/filters/#boolean-operators
			 */
