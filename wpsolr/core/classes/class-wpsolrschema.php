...
class WpSolrSchema {
	 ...
	 * @throws \Exception
	 */
	public static function get_sanitized_value( WPSOLR_AbstractIndexClient $search_engine_client, $field_name, $value, $post, $is_relation ) {
		...
		try {
			...
			if ( null === $sanitized ) {
				...
				if ( $is_relation ) {
					$values = self::get_sanitized_json_value( $search_engine_client, $field_name, $value, $field_type, $post );
				} else {
